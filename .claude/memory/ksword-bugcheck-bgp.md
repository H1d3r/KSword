...
- `bugcheck_layout.c/.h` 是 BGP 与 VMware SVGA 共用的信息架构：深海军蓝底、蓝色强调、浅色正文、弱化标签和一像素分区边框；渲染器提供 `DrawText/DrawHeroText/DrawFrame` 回调，停止码数值使用独立的 2 倍字形通道。
...
- BGP 的 2 倍字号字形与普通字形一样，必须在 `PASSIVE_LEVEL` 预生成；崩溃回调只能绘制已经解析的矩形。VMware SVGA 可直接以 scale=2 绘制，但必须使用同一布局坐标和色彩角色。
