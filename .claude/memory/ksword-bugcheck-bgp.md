...
- `1280x720` 信息完整版使用 11 个预生成分区框；线程/进程、栈展开、指令字节、Windows Blackbox、其他 CPU 状态、事件时间和百分比只展示可用性及分析命令，不在高 IRQL 回调中强行采集。`DUMP ONLY` 或 `NOT CAPTURED` 使用弱化灰色，避免与红色停止码争抢视觉焦点。
- 诊断配色保持克制：红色只用于页眉与 BugCheck 面板中的停止码名称/数值，蓝色仅作标题或已解码值的轻量强调；已捕获状态显示正文白色，依赖转储的内容显示弱化灰色，不再同时使用红、黄、绿、蓝争抢注意力。BGP 的 24/32 BPP 字形缓存和 VMware SVGA 调色板必须同步。
- 三种固定布局都必须把 BugCheck 名称与 `STOP CODE 0x........` 放在页眉第一视觉层；完整版和详细版的右上角保留原 8x12 白色玩笑文案，并在最右侧单独显示 `CRASH CPU` 与 `IRQL`。640x480 紧凑版空间不足时保留第一句玩笑，但 CPU/IRQL 仍固定在最右侧。
