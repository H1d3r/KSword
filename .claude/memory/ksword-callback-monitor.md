...
- R0 在 callback runtime 内预分配 2048 条固定记录。发布者只读取类别原子掩码、try-lock 单写者并复制到固定槽；争用时计入 dropped，不等待、不分配。Start/Stop 控制线程可以等待该短临界区，避免 ring 重置或停止与尾部提交竞态。
...
- R3 只通过 `ArkDriverClient` 控制和读取，并严格校验 ABI 尺寸、版本、返回字节数、entrySize、游标范围和事件序号连续性。旧驱动的未知 IOCTL 映射成明确的 unsupported 状态。
- MonitorDock 页面按 Tab 懒加载，打开不自动采集。高频事件使用追加式 `QAbstractTableModel` + `QSortFilterProxyModel`，后台线程持独立游标；暂停只冻结 100 ms UI 批量提交，后台继续读入有界队列。右键菜单打开期间不得更新模型。
